import React, { useContext, useState } from 'react'
import { useTranslation } from 'react-i18next'
import { Link } from 'react-router-dom'
import { mainContext } from '../context/MainContext'
import { auth } from '../firebase/firebase.config'
import { signOut } from 'firebase/auth'
import toast from 'react-hot-toast'


const languages  = [ 
  {
    id: 1,
    icon: "/uzbekistan.png",
    text: "uz",
  },
  {
    id: 2,
    icon: "/russia.png",
    text: "ru"
  },
  {
    id: 3,
    icon: "/britain.png",
    text: "en"
  },
]

function Navbar() {
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "light");
  const { 
    state: { userInfo },
    dispatch,
  } = useContext(mainContext);
  const { t, i18n } = useTranslation();

  const logout = () => {
    signOut(auth)
      .then(() => {
      toast.success(t("chiqishToast"));
    })
    .catch((error) => {
      toast.error(t("chiqishyakunlanmadi"));
    });
    dispatch({ type: "LOGOUT" });
  };

  const changeLanguage = (lang) => {
    i18n.changeLanguage(lang.text).then(() => {
      toast.success(i18n.t("tilozgartirildi"));
    });
  };
  

  const changeTheme = () => {
    setTheme((prev)=> {
      let newTheme =  prev == "light" ? "dark" : "light";
      localStorage.setItem("theme", newTheme)

      return newTheme
    });
  };


  return (
    <div className="navbar bg-base-100 shadow-sm shadow-gray-200">
        <div className="container flex justify-between items-center">
                {/* <Link className="btn btn-ghost text-xl" to="/">
                <img src="/navo.logo.jpg" alt="site logo" width={28} height={28} />
                   Navo.ai
                </Link> */}
                <Link className="btn btn-ghost text-xl flex items-center gap-2" to="/">
                <div className="w-7 h-7 rounded-full overflow-hidden">
                  <img src="/navo.logo.jpg" alt="site logo" className="w-full h-full object-cover" />
                </div>
                  Navo.ai
                </Link>

                <div className="flex items-center gap-4">
                <label className="swap swap-rotate">
                  <input
                    onClick={changeTheme}
                    type="checkbox"
                    value="dark" 
                    defaultChecked = {theme == "light" ? false : true}
                    className="theme-controller" 
                  />
                  <svg
                    className="text-yellow-400 swap-off fill-current"
                    width={28}
                    height={28}
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24">
                    <path
                      d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
                  </svg>
                
                  <svg
                    className="text-blue-400 swap-on fill-current"
                    width={28}
                    height={28}
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24">
                    <path
                      d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
                  </svg>
                </label>
                    <div className="dropdown dropdown-end">
                      <div 
                        tabIndex={0} 
                        role="button" 
                        className="btn m-1"
                      >
                        <img 
                          src={languages.find((lang)=> lang.text === i18n.language.split("-")[0]). icon} 
                          alt={i18n.language}
                          width={24}
                          height={24}
                        />
                        <span>{i18n.language.split("-")[0]}</span>
                      </div>
                      <ul 
                        tabIndex={0} 
                        className="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-xl"
                      >
                        {languages.map((lang)=> {
                          return (
                            <li key={lang.id}>
                              <button onClick={()=> changeLanguage(lang)}>
                                <img 
                                  src={lang.icon} 
                                  alt={lang.text}
                                  width={24}
                                  height={24}
                                />
                                <span>{lang.text}</span>
                              </button>
                            </li>
                          );
                        })}
                      </ul>
                    </div>
                    <div className="flex-none">
                        <div className="dropdown dropdown-end">
                            <div 
                              tabIndex={0} 
                              role="button" 
                              className="btn btn-ghost btn-circle avatar">
                                <div className="w-10 rounded-full">
                                    <img
                                      alt="user avatar"
                                      src={userInfo.photoURL} />
                                </div>
                            </div>
                            <ul
                               tabIndex="-1"
                               className="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow"
                            >
                                <li>
                                  <button className="justify-between">
                                    {t("profil")}
                                  </button>
                                </li>
                                <li>
                                  <button>{t("sozlamlar")}</button>
                                </li>
                                <li>
                                  <button onClick={logout}>{t("chiqish")}</button>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
        </div>
    </div>
  )
}

export default Navbar;